...
String Server_ExecuteRequest(int servertype, int request, String ArduinoRequest);
...
void SdkWebServer_recv(void *arg, char *pusrdata, unsigned short length)
{
    ...
    /*** DEBUG CODE to verify parsed URL parameters
    for(i=0;i<pURL_Param->nPar;i++) {
        Serial.print("Param");
        Serial.print(i+1);
        Serial.print(": ");
        Serial.print(pURL_Param->pParam[i]);
        Serial.print("  Value: ");
        Serial.println(pURL_Param->pParVal[i]);
    }
    */
    char * pRx = NULL;
    ...
    switch (pURL_Param->Type) {
        case GET:
            ...
            if(os_strcmp(pURL_Param->pParam[0], "arduino")==0) {
                ...
                //-------------- Request = GetDigital ----------------------
                if(os_strcmp(pURL_Param->pParVal[0], "GetDigital")==0){
                    if(os_strcmp(pURL_Param->pParam[1], "chan")==0) {
                ...
                if(os_strcmp(ArduinoRequest.c_str(), "Invalid Request")==0) {
                    ...
                    payld = Server_ExecuteRequest(SVR_HTTP_SDK, ARDUINO_REQUEST, ArduinoRequest);
                }
                SdkWebServer_senddata(ptrespconn, true, (char *)payld.c_str());
            }
            break;
 ...
 * Function: Server_ExecuteRequest(int servertype, int request, String ArduinoRequest)
 * 
 * Parameter      Description
 * ---------      ---------------------------------------
 * servertype     server type (SDK or LIB)
 * request        request enumberation
 * ArduinoRequest request string to send to Arduino
 * return         reply string
 ********************************************************/
String Server_ExecuteRequest(int servertype, int request, String ArduinoRequest) {
    String s = "";  
    ...
    switch (request) {
        ...
        case ARDUINO_REQUEST:
            s = ArduinoSendReceive(ArduinoRequest);
            break;
        default:
            break;
   }
